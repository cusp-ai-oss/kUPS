...
def max_dof_per_system(
    ...
    *,
    ...
    runtime_assert(
        ~jnp.any(jnp.isposinf(result.data)), "Non-finite relaxation gradients."
    )
    return result
...
def converged_per_system(
    ...
    *,
    ...
    return max_dof_per_system(
        gradients, index_prefix, include_cell=include_cell
    ).map_data(lambda m: m < tolerance)
