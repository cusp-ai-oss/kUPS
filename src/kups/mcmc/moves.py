...
class ExchangeChanges:
    ...
    def is_noop(self, groups: Table[GroupId, HasMotifAndSystemIndex]) -> Array:
        ...
        old_system, new_system = Index.match(current.system, proposed.system)
        old_motif, new_motif = Index.match(current.motif, proposed.motif)
        same_group = ~self.groups.indices.valid_mask | (
            (old_system == new_system) & (old_motif == new_motif)
        )
